function [x, options, flog, pointlog, scalelog] = scgMixed(f, x, options, gradf, varargin)
% Scaled conjugate gradient optimization for multigpMixed.
%
%	Description
...
%   This function is modified from SCG by Yehong for fitting the
...
sigma0 = 1.0e-4;
[fold, model] = feval(f, x, varargin{:});	% Initial function value.

fnow = fold;
options(10) = options(10) + 1;		% Increment function evaluation counter.
gradnew = feval(gradf, x, model);	% Initial gradient.
gradold = gradnew;
...
while (j <= niters)
  ...
  if (success == 1)
    ...
    xplus = x + sigma*d;
    gplus = feval(gradf, xplus, model, 1);
  ...
  xnew = x + alpha*d;
  [fnew, modelnew] = feval(f, xnew, model);
...
%     if (max(abs(fnew-fold)) < options(3))
      options(8) = fnew;
      return;
